...
export default async function BillingPage({
  ...
  let activePlan = plan;
  if (!activePlan && allPlans && allPlans.length > 0) {
    activePlan = allPlans.find(p => p.name.toLowerCase().includes('professional')) || allPlans[0];
  }
    ...
    <div className="space-y-6 max-w-[1600px]">
      ...
      <BillingClient
        ...
        usage={{
          ...
          maxSchools: activePlan?.max_schools ?? 10,
          staff: staffCount ?? 0,
          maxStaff: activePlan?.max_staff ?? 50,
          students: 0,
          maxStudents: activePlan?.max_students ?? 500,
        }}
