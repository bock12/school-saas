...
export function ParentsDashboardClient({ totalParents }: { totalParents: number }) {
  const params = useParams();
  const router = useRouter();
  const tenant = params.tenant as string;

  const quickActions = [
    { label: 'Add Contact', desc: 'Register parent or guardian', href: `/admin/parents/contacts`, icon: UserPlus, primary: true },
    { label: 'Link Contact', desc: 'Link contact to student', href: `/admin/parents/relationships`, icon: Layers },
    { label: 'Invite Parent Portal', desc: 'Dispatch portal invite email', href: `/admin/parents/portal-approvals`, icon: Mail },
    { label: 'Print Contact Cards', desc: 'Batch ID card print', href: `/admin/parents/bulk`, icon: LayoutGrid },
    { label: 'Bulk Import', desc: 'Upload CSV templates', href: `/admin/parents/bulk`, icon: Download },
    { label: 'Send Announcement', desc: 'Dispatch broadcast to contacts', href: `/admin/parents/communication`, icon: MessageSquare },
    { label: 'Generate Family Report', desc: 'Consolidated sibling lists', href: `/admin/parents/reports`, icon: FileText }
  ];
  ...
  return (
    <div className="space-y-8 max-w-[1600px] animate-fade-in px-4 sm:px-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Total Registered Adults', value: totalParents.toString(), sub: 'Parents & Guardians', change: '+3.1%', icon: Users, color: 'from-blue-500/20 to-blue-600/5 border-blue-500/30 text-blue-400' },
