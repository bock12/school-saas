          ...
          <div key={dept}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 mb-6">
              {subjects.map((subject, i) => (
                <div key={subject.id} className="glass-card p-4 flex items-center justify-between group animate-fade-in" style={{ animationDelay: `${i * 40}ms` }}>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-[hsl(var(--text-primary))]">{subject.name}</p>
                      <div className="flex items-center gap-2 mt-0.5">
                        <code className="text-[10px] font-mono text-[hsl(var(--text-tertiary))]">{subject.code || 'N/A'}</code>
                        {subject.is_elective && <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-400 font-medium">Elective</span>}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
